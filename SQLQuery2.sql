...
select Min(Age) as Min_age,Max(Age) as Max_age,STDEV(Age) as stddev from users



select max(t) as Counter1 from (select count( FollowedUsername) as t from Following
group by FollowedUserName) as t

select UserName from Users except select Username from Tweets

select Username from Tweets except select Username from tweets where Text like '%#Census%';

select UserName from Users except select FollowedUserName from Following;

select UserName from Users where NOT EXISTS
(SELECT * from Following where Users.UserName = Following.FollowedUserName);


select a.country from Users as a Inner Join Tweets as t on a.UserName=t.UserName
group by a.country
order by a.Country asc

select f.FollowedUserName from following as f inner join users as u on f.FollowerUserName=u.UserName
